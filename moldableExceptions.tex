...
\ins{For example in a parsing framework an exception can indicate that a given input cannot be parsed.}
...
\ins{While in this introductory example, the difference is easy to stop, in other cases like the ones presented in} \autoref{sec:casestudy}, \ins{a diff view is essential in finding differences.}
...
\begin{enumerate}[(i)]
	...
	\item enable automated fixes (code \ins{and data} transformations) for common programming errors. 
\end{enumerate}

We will illustrate these three points with several examples.
In \autoref{sec:inspectorViews} we introduce the \emph{moldable inspector}, which allows objects to define custom views \ins{and actions} when they are inspected.
In \autoref{sec:views} we show how custom debugger views can easily be defined in a similar way by adding simple annotated view \ins{and actions} methods to the class of a moldable exception.
...
In \autoref{sec:casestudy} we describe a case study in which we used moldable exceptions to improve a development phlow.
...
When a moldable object inspector is opened on an object it looks for any view \ins{and action} methods defined in the class of that object, and uses those methods to create custom views.
...
Same as views, objects can define actions for performing relevant operations. For example, the game objects in \autoref{fig:ludoBoardView} defines a \lst{Autoplay one move} action (having the icon \smallimg{ludoPlayActionIcon}) that can be used to simulate a player's turn. \ac{see later one how to get a less pixelated icon}
...
However this time we show the \lst{Textual Diff} view both in the new debugger interface created for this exception (\lst{Textual Diff} tab in \autoref{fig:scripterPreview}), as well as an additional view within the standard \GT debugger \GT debugger (\lst{Textual Diff} tab in \autoref{fig:scripterDiffInDebugger}). Hence, a moldable exception can also provide views and actions shown in the standard \GT debugger.
...
A custom debugger action, such as \emph{Fix \& retry} or \emph{Retry} is defined in the same way as a custom debugger view: it is a method with a particular annotation, in this case being \st{<gtPhlowSameViewReturnDebugAction>} rather than \mbox{\st{<gtExceptionView>}}, but  the mechanism is essentially the same. We see the code of this action in the listing below. Since this is a debugging action we need to be able to control the run-time execution. To make this possible, methods creating debugging views and actions can take as a second parameter a context objects that gives them access to the current run-time execution.
...
\begin{figure*}[h]
  \includegraphics[width=\textwidth]{comparisonFailureTextualDiff}
...
\begin{figure*}[h]
  \includegraphics[width=\textwidth]{comparisonFailureSourceDiff}
...
Below we see the method in the \lst{Exception} class that defines a custom debugger for every exception.
...
By default, as mentioned in \autoref{sec:views}, the debugger looks for methods  annotated with a \lst{<gtExceptionView>} pragma. 
...
We can see this in \autoref{sec:casestudy} \ins{where the source of the diff view and accept action are defined in class \lst{TextOrVisualComparisonType}.
 The ComparisonFailure exception defines the following debugger specification that uses a different pragma to locate views and adds also another object as a target for locating actions and views.}
...
\ins{To enable moldable exceptions to perform automated transformations instead of opening the debugger, we provide a dedicated trait that exceptions can use. This trait provides an implementation for the signal method shown below. This method is executed when an exception is raised. By default in case an exception is not handled in the system a debugger is opened. This override checks if automatic transformations are allowed for the current exception, and it they are, asks the exception for its transformation and executes it case there is anything to transform. Here transformations can apply code changes, modify the state of objects, or change  other changes, like for example deleting files from disk.}
...
While both the moldable debugger and moldable inspector were available in Glamorous Toolkit at the same time, until now Glamorous Toolkit comes with 3200 views and 280 actions with an average of 12 lines of code, but only 8 custom debuggers (like the one in  \autoref{fig:smaccDebugger}). 

This indicates that the extension mechanism plays a significant role in developers extending the IDE, and that the cost of creating views and actions can be very low. Due to this, through moldable exceptions we aim to apply the same extension mechanism present in the moldable inspector for creating custom debuggers.
...
%\ac{We could also mention here that the moldable debugger and inspector were added in GT at around the same time and had two different extension mechanism: subclassing vs. adding extension methods. And until now we created just around 8 debuggers but 3000+ views. }
