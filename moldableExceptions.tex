...
%\tk{Scope: never stated we mean *interactive debuggers* vs automated debugging etc.}
...
It will typically take a developer some time to putter around in the debugger interface to understand the specific error (the strings do not match), and why the strings do not match.
...
% \tk{Background on moldable tools was missing.}
...
A good example of such a tool is a \emph{moldable object inspector}~\cite{Chis15a}.
...
\end{figure}
%\tg{I would change this picture with two separate windows showing the two views: this is because people typically associated adjacent panes with some connection - it seems harder for people to grasp that we can spawn the same object there, too. Also, there should be no highlighting in the Raw view.}
...
\end{figure}
% \tg{I think here we could show the default debugger with the custom Textual Diff view on the right as an example of how we can extend the default debugger as well.}
...
%  \caption{The trouble with the generic debugger view.}
...
\todo{Add some anecdotal evidence of the impact of moldable exceptions in the Lifeware diff tests. }

%\tk{Focus was on implementation effort, but not on impact.
%Does it really help in debugging?
%Is there some anecdotal evidence?}
%\on{Are there Lifeware anecdotes?}
%\tk{A study with students and the Ludo game could be interesting.
%One group with and another without the moldable debugger.}
...
\on{FIX redundancy with reference in \autoref{sec:inspectorViews}.}
