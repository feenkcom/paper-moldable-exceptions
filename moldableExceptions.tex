...
\todo{The deadline for the final version is September 1, and we will contact you later about how to submit it.}

\on{
Most positive comments have been removed.
Only potentially actionable comments have been kept here.
Full review comments are in the review subfolder.
}

\rA{... it would be nice if the screenshots were bigger.}
\on{I will prepare a single-column version for arxiv.org}
...
\on{AC -- can you provide some anecdote about your experience building custom debuggers?}

\rC{- I love line 1122 ([``Debuggers are unloved beasts.'' in conclusion] and subsequent prose)! The very first paragraph of the intro didn’t quite land with me (I still use a terminal-based debugger and kinda like it… but I agree that we need reimagined debuggers), so maybe you could consider incorporating the perspective/prose from 1122 in the first paragraph of the intro?}
\todo{Update the intro accordingly.}
...
\ins{These methods are annotated with a \lst{<gtExceptionView>} pragma for views and a \lst{<gtExceptionAction>} pragma for actions to extend the moldable debugger
(\ie as opposed to \st{<gtView>} and  \st{<gtAction>} which extend the moldable inspector).}

\rB{* (l. 284-285) Here, the authors talk about the \lst{<gtView>} and \lst{<gtAction>} annotations, but then change to presumably more specific variants like \lst{<gtExceptionView>} etc. Readers might be wondering about the relationship between those.}\on{fixed}
\ac{added a small change to also mention gtExceptionAction}
...
\rB{* (l. 328…) What is view forwarding / how does it work?}\on{fixed}
\ins{Specifically, the moldable debugger game view for a \lmaf simply reuses the moldable inspector view called \st{gtPositionsFor:} that is already defined in the \st{game} object.}
\begin{code}
gtGameViewFor: aView
	<gtExceptionView>
	^ aView forward
...
\rB{* (l. 336) The need to explicitly provide / decide on priorities could be a source of many problems.  Why are they there, how can one take advantage of them, and what needs to be considered/ensured/avoided in a production-level debugger environment based on them?}
\on{I suggest we ignore this point.}
\ac{I do not fully understand that comment. We use priorities as a way to sort and it was never an issue}
...
\rC{- The code snippets started getting hard to follow starting on line 397. Syntax highlighting might help, or spending a bit more time explaining the code (or just removing anything unnecessary).}\on{Just highlighted the test in bold}
\begin{code}
...
\rB{* (l. 416…) Section 4 built around “GT Scripter” felt a little out of place compared to the text until then and should be revised.}
\on{Not sure what is needed.}
\ac{I think it is ok like it is now}
...
\begin{inparaenum}[(i)]
	...
	\item We check that a new page is created \chg{(\ie the \emph{Coder} for the \st{Object} class).}{with a code browser for the \st{Object} class.}
	\rC{- Line 428: What is the “Coder”?}\on{fixed}
\end{inparaenum}
...
\rC{\# 2. What is the benefit of moldable exceptions specifically (vs. other moldable interfaces)?\\
...
Although it is possible to get at the information we seek, doing so is clumsy \ins{since we would have to navigate to the exception instance itself, and explore its  internal state}, and the classical stack view only confuses matters instead of helping us to debug the problem.
\rC{- Line 508 regarding “doing so is clumsy”: Make that clumsiness tangible! If you really show the reader how clumsy it would be to do the alternative, that would be a great opportunity to highlight what works well about your approach. As of right now, I am basically just taking your word on it.}\on{Added a brief explanation}
...
\rB{* (l. 561…) The first four paragraphs of Section 5 felt overly specific and, as presented, not contributing to the argument.}
\on{Commented out the paragraphs discussing Pharo deprecated  methods -- perhaps we can mention them instead in related work.} 
\ac{yes, we can mention it in the related work}
...
%	% \rB{* (l. 567) “an explanatory string, and” -> “an explanatory string and”}
...
%
Consider the case
% of a common programming error being that of 
...
% \rB{* (l. 601) “caught a static type system” -> “caught by a static type system”}
...
\ins{Given how GT inspector views are executed, in this particular case, after applying the transformation that fixes the code of the view, it is enough to restart the top call frame. In other cases it might be needed to walk over the call stack to find the right place from were to restart execution.}

A custom debugger action, such as \emph{Fix \& retry} or \emph{Retry} is defined in the same way as a custom debugger view: it is a method with a particular annotation, in this case being \chg{\st{<gtPhlowSameViewReturnDebugAction>}}{\st{<gtExceptionAction>}} rather than \mbox{\st{<gtExceptionView>}}, but  the mechanism is essentially the same. We see the code of this action in the listing below.
...
\ac{We now use <gtExceptionAction> as an annotation for actions. Initially the mechanism was a bit different}
...
\end{code}
\chg{The \st{signal} method of exception classes}{The mechanism for managing unhandled exceptions} can then optionally consult this flag to decide whether to spawn a debugger or apply the fix (\autoref{sec:implementation} gives more details).
...
\rA{* It would be nice to know a bit more about how the implementation works (the current "Implementation" section gives some hints, but seems more about the specific API of the system). More specifically, I wonder what all the information of the last 5 lines in the code snippet (gtDebugActionFixItFor) on line 685-697 mean - and what other options are there. 
Another case is "Debugging Targets" (line 884) which also seems a very important technical aspect - but you do not say how this is used/set in specific case studies in the paper.}
\on{AC -- do you want to add something?}
...
\rB{* (l. 950) “self signal” -> “super signal”?}
...
\rC{- The SmaCC reference was a bit confusing and not very fleshed out - I would recommend either spending some more space explaining it or simply removing it.}
...
3. Teitelman, Warren. "Interlisp Reference Manual. Xerox Corporation (1974).}
% Available at:  http://bitsavers.informatik.uni-stuttgart.de/pdf/xerox/interlisp
% Interlisp_Reference_Manual_1974.pdf (see Section 17)

\on{AC -- which of these do you feel are worth citing?}

\todo{To cite?:
\cite{Rein24a}
\cite{Teit66a}
\cite{Teit74a} \\
NB: PDFs at \url{https://www.dropbox.com/home/feenk/feenk-docs/MoldableExceptions}
}

\rB{I would like to suggest revisiting related work wrt. to projectional editors, Babylonian programming, and live and exploratory programming in general.}

\todo{Search for suitable references ... ?}

\rC{- The related work feels a bit like a potpourri of other points in the space. Would it be possible to impose some sort of hierarchy or other structure into the review, perhaps by setting up axes in which these tools vary or are the same? I feel like I would come away with a better understanding of where moldable exceptions fits into the broader landscape of debugging tools.}
\on{Reviewer C is not wrong, but I'm not sure what to do.}

\rC{- Also regarding the related work: Are “expect tests” (a.k.a. “snapshot tests” or “golden tests”) relevant to discuss? They are also often based on exceptions and diffs. https://blog.janestreet.com/the-joy-of-expect-tests/}
...
\rC{- I love line 1122 (``Debuggers are unloved beasts.'' and subsequent prose)! The very first paragraph of the intro didn’t quite land with me (I still use a terminal-based debugger and kinda like it… but I agree that we need reimagined debuggers), so maybe you could consider incorporating the perspective/prose from 1122 in the first paragraph of the intro?}
\todo{I can move this text to the intro and paraphrase it here.}
