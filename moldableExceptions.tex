...
As a case study, we applied moldable exceptions in a company developing applications for managing insurance policies to improve the developmnt process by integrating an external tool into the debugger. 

One part of the application consists in generating various documents related to insurance contracts. 
The creation of documents is verified using tests checking that recreating a document matches an expected version saved in the system. 
When a comparison fails a developer need to assess the differences between the two documents and then in case those differences are expected, update the expected document with the obtained one, or in case they are not expected, fix the route cause of the difference.

A developer can use a dedicated tool to see the diff between the obtain and the expected document, and in case the difference is ok update the expected document with the new one. 
In case the difference indicates a bug, the developer has to run again the test using the debugger to explore the live execution. In this flow developers have to choose the tool before they run the test, which means that they typically ran a test twice. 
This can be a problem as some tests take minutes to run.

With moldable exceptions we integrated views showing differences between the obtained and the expected documents, and actions to update the expected document with the new one and resume executing the test, directly in the debugger. 
To achieve this we rely on the fact that tests raise \lst{ComparisonFailures} exceptions when two documents to not match.
...
Figure \autoref{fig:comparisonFailureVisualDiff}  shows the debugger opened on a \lst{ComparisonFailures} raised as a result of a failed comparison two pdf documents. 
In this case the exception provides several diff views: one at the image level highlighting visually the parts that are different between the two documents (Figure \autoref{fig:comparisonFailureVisualDiff}), one only at the level of the text contained by the PDF documents (Figure \autoref{fig:comparisonFailureTextualDiff}), and one at the level of the raw document format. 
Apart from the diff views, the exception also provides an Accept action that updates the expected document with the new one and continues the execution of the test.

We see below the source code of the `Image Diff` and Listing Y the source for of the 'Accept' action. 
In both cases they reuse existing logic and within a few lines of code integrate it in the debugger. 
This integration unifies the flow of comparing two documents and exploring why the failure happen using a debugger. 
Developer can now simply run tests and when a comparison fails choose how to proceed.

\begin{code}
add code for the Diff view
\end{code}

\begin{code}
add code for the Accept actions
\end{code}
...
\begin{code}
gtExceptionDebuggerSpecification
