...
Only actionable comments have been kept here.
...
\rB{* (l. 284-285) Here, the authors talk about the \lst{<gtView>} and \lst{<gtAction>} annotations, but then change to presumably more specific variants like \lst{<gtExceptionView>} etc. Readers might be wondering about the relationship between those.}
...
gtGameViewFor: aView
	...
	^ aView forward
		...
		view: #gtPositionsFor:
\end{code}
\rB{* (l. 328…) What is view forwarding / how does it work?}
...
\rC{- The code snippets started getting hard to follow starting on line 397. Syntax highlighting might help, or spending a bit more time explaining the code (or just removing anything unnecessary).}
\begin{code}
...
\rC{- Line 508 regarding “doing so is clumsy”: Make that clumsiness tangible! If you really show the reader how clumsy it would be to do the alternative, that would be a great opportunity to highlight what works well about your approach. As of right now, I am basically just taking your word on it.}
